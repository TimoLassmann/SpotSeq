...
int make_model(struct parameters* param,struct seq_buffer* sb)
{
        ...
        model->expected_K = 2;
        //DPRINTF2("START PGAS.");
        // RUN(iHmmSampleBeam(model, tmp_seq_pointer,sb->num_seq))
...
int make_dot_files(struct parameters* param)
{
        ...
        snprintf(buffer_out, BUFFER_LEN, "%s/%s/%s",param->outdir,OUTDIR_VIZ,"test_dotfile.dot");

        RUN(make_dot_from_matrix(buffer_in,buffer_out));
        return OK;
ERROR:
        return FAIL;
}
